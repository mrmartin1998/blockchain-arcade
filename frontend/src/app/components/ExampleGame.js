...
import { ExampleGame, ArcadeToken } from '../utils/contract';
...
const ExampleGameComponent = () => {
    ...
    const handlePlayGame = async () => {
        ...
        try {
            ...
            let allowance = new BigNumber(await ArcadeToken.methods.allowance(accounts[0], ExampleGame._address).call());
            console.log('Allowance:', allowance.toString());

            if (allowance.isLessThan(gameCost)) {
                // Approve the ExampleGame contract to spend the required amount of tokens
                await ArcadeToken.methods.approve(ExampleGame._address, gameCost.toString()).send({ from: accounts[0] });
                console.log('Approval transaction sent.');

                // Re-check the allowance after approval
                allowance = new BigNumber(await ArcadeToken.methods.allowance(accounts[0], ExampleGame._address).call());
            ...
            // Estimate gas for the play method in ExampleGame
            let gasEstimate;
            try {
                gasEstimate = await ExampleGame.methods.play().estimateGas({ from: accounts[0] });
            ...
            await ExampleGame.methods.play().send({ from: accounts[0], gas: gasEstimate });
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-base-200">
            <div className="bg-base-100 p-8 rounded shadow-md w-full max-w-md text-center">
                <h1 className="text-2xl font-bold mb-6">Play Example Game</h1>
